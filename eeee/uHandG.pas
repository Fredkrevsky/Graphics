...
THuman = class
    ...
    const arm1 = 60;
      ...
      constructor Create;
    public
      procedure setrarm(x, y:Integer; clockwise:boolean);
      procedure setlarm(x, y:Integer; clockwise:boolean);
      procedure setrleg(x, y:Integer; clockwise:boolean);
      procedure setlleg(x, y:Integer; clockwise:boolean);
...
begin
  ...
  with source.canvas do
    begin
      ...
      PolyLine([center+Bodydown, center+FindThirdPoint(Bodydown, rightleg, leg1, leg2, rlclock, false),center+rightleg]);
      PolyLine([center+Bodydown, center+FindThirdPoint(Bodydown, leftleg, leg1, leg2, llclock, false),center+leftleg]);
      temp:=center+FindThirdPoint(Bodyup, leftarm, arm1, arm2, laclock, true);
      PolyLine([center+Bodyup, temp]);
      PolyLine([temp,center+leftarm]);
      Draw(x-100+xc,y-100+yc, guitarsource);
      PolyLine([center+Bodyup, center+FindThirdPoint(Bodyup, rightarm, arm1, arm2, raclock, true),center+rightarm]);
...
begin
  ...
  with source.canvas do
    begin
      ...
      PolyLine([center+bodydown, center+bodyup]);
      Ellipse(Rect(center+BodyUp+Point(-headr, -headr*2),center+BodyUp+Point(headr, 0)));
      PolyLine([center+Bodyup, center+FindThirdPoint(Bodyup, rightarm, arm1, arm2, raclock, false),center+rightarm]);
      PolyLine([center+Bodyup, center+FindThirdPoint(Bodyup, leftarm, arm1, arm2, laclock, false),center+leftarm]);
      PolyLine([center+Bodydown, center+FindThirdPoint(Bodydown, rightleg, leg1, leg2, rlclock, false),center+rightleg]);
      PolyLine([center+Bodydown, center+FindThirdPoint(Bodydown, leftleg, leg1, leg2, llclock, false),center+leftleg]);
    end;
end;
